...
public class TFYSwIFTEasyBlueToothManager: NSObject {
    ...
    public var managerOptions: TFYSwiftEasyManagerOptions? = TFYSwiftEasyManagerOptions(queue: nil)

    /// 连接设备的时候蓝牙状态发生改变
    public typealias blueToothStateChanged = (_ peripheral:TFYSwiftEasyPeripheral?,_ state:bluetoothState) -> Void

    /// 模糊搜索设备规则
    /// 用户可以自定义，依据peripheral里面的名称，广播数据，RSSI来赛选需要的连接的设备
    public typealias blueToothScanRule = (_ peripheral:TFYSwiftEasyPeripheral) -> Bool

    /// 第一种：扫描到符合条件的单个设别就立马回调。
    /// 第二种：在规定的时间里扫描出所有和服条件的设备，它会等到所规定的时间完成才会回调。
    public typealias blueToothScanCallback = (_ peripheral:TFYSwiftEasyPeripheral?,_ error:Error?) -> Void
    public typealias blueToothScanAsyncCallback = (_ peripheral:TFYSwiftEasyPeripheral?,_ type:searchFlagType,_ error:Error?) -> Void
    public typealias blueToothScanAllCallback = (_ deviceArray:[TFYSwiftEasyPeripheral],_ error:Error?) -> Void

    /// 连接设备的回调
    /// 说明：error为nil的时候说明连接成功。如果error里面有值，请做相应的处理
    public typealias blueToothConnectCallback = (_ peripheral:TFYSwiftEasyPeripheral?,_ error:Error?) -> Void

    /// 读/写/监听特征 操作回调
    public typealias blueToothOperationCallback = (_ value:Any?,_ error:Error?) -> Void

    /// 寻找特征的回调
    public typealias blueToothFindCharacteristic = (_ character:TFYSwiftEasyCharacteristic?,_ error:Error?) -> Void

    /// 方法一：外部可以KVO监听bluetoothState值的改变。
    /// 方法二：bluetoothStateChanged 实现这个block回到
    public var bluetoothState:bluetoothState?
    public var bluetoothStateChanged:blueToothStateChanged?

    /// 单例
    public static var shareInstance:TFYSwIFTEasyBlueToothManager {
        struct Static {
    ...
    public func scanDeviceWithName(name:String,callback:blueToothScanCallback?) {
        self.scanDeviceWithCondition(condition: name, callback: callback)
    }
    public func scanDeviceWithRule(rule:blueToothScanRule?,callback:blueToothScanCallback?) {
        self.scanDeviceWithCondition(condition: rule, callback: callback)
    }

    /// 扫描符合规则的全部设备 ---> 发现一个回调一个。当到规定的时间停止扫描。
    public func scanAllDeviceAsyncWithRule(rule:blueToothScanRule?,callback:blueToothScanAsyncCallback?) {
    ...
    public func scanAllDeviceWithName(name:String,callback:blueToothScanAllCallback?) {
        self.scanAllDeviceWithCondition(condition: name, callback: callback)
    }
    public func scanAllDeviceWithRule(rule:blueToothScanRule?,callback:blueToothScanAllCallback?) {
    ...
    public func connectDeviceWithIdentifier(identifier:String,callback:blueToothConnectCallback?) {
        ...
        if let connectedPeripheral = self.centerManager.connectedDeviceDict[UUIDString] as? TFYSwiftEasyPeripheral {
            callback?(connectedPeripheral,nil)
        } else if let foundPeripheral = self.centerManager.foundDeviceDict[UUIDString] as? TFYSwiftEasyPeripheral {
            self.connectDeviceWithPeripheral(peripheral: foundPeripheral) { peripheral, error in
                callback?(peripheral,error)
            }
        } else {
            self.scanDeviceWithRule { peripheral in
                return (peripheral.identifierString?.contains(UUIDString)) ?? false
                ...
                if error != nil {
                    ...
                    self.connectDeviceWithPeripheral(peripheral: peripheral) { peripheral, error in
                        callback?(peripheral,error)
                    }
                }
            }
    ...
    public func connectDeviceWithPeripheral(peripheral:TFYSwiftEasyPeripheral?,callback: @escaping blueToothConnectCallback) {
        guard let peripheral = peripheral else {
        ...
        // 检查是否已经连接
        for tempP2 in self.centerManager.connectedDeviceDict.values {
            if let tempP = tempP2 as? TFYSwiftEasyPeripheral, tempP.isEqual(peripheral) {
        ...
        peripheral.connectDeviceWithTimeOut(timeout: self.managerOptions?.connectTimeOut, options: self.managerOptions?.connectOptions, callback: { [weak self] perpheral, error, type in
            ...
            case .deviceConnectTypeDisConnect:
                ...
                if self?.managerOptions?.autoConnectAfterDisconnect == true {
    ...
    public func scanAndConnectDeviceWithName(name:String,callback:blueToothScanCallback?) {
    ...
    public func scanAndConnectDeviceWithRule(rule:@escaping blueToothScanRule,callback:blueToothScanCallback?) {
    ...
    public func scanAndConnectDeviceWithIdentifier(identifier:String,callback:blueToothScanCallback?) {
        self.connectDeviceWithIdentifier(identifier: identifier, callback: callback)
    }

    /// 连接已知名称的所有设备（返回的是一组此名称的设备全部连接成功）--->（慎用此功能）
    public func scanAndConnectAllDeviceWithName(name:String,callback:blueToothScanAllCallback?) {
    ...
    public func scanAndConnectAllDeviceWithRule(rule:blueToothScanRule?,callback:blueToothScanAllCallback?) {
    ...
    public func writeDataWithPeripheral(peripheral:TFYSwiftEasyPeripheral,serviceUUID:String,writeUUID:String,data:Data,callback:blueToothOperationCallback?) {
    ...
    public func readValueWithPeripheral(peripheral:TFYSwiftEasyPeripheral,serviceUUID:String,readUUID:String,callback:blueToothOperationCallback?) {
    ...
    public func notifyDataWithPeripheral(peripheral:TFYSwiftEasyPeripheral,serviceUUID:String,notifyUUID:String,notifyValue:Bool,callback:blueToothOperationCallback?) {
    ...
    /// 对描述进行操作。
    public func writeDescriptorWithPeripheral(peripheral:TFYSwiftEasyPeripheral,serviceUUID:String,characterUUID:String,data:Data,callback:blueToothOperationCallback?) {
        self.searchCharacteristicWithPeripheral(peripheral: peripheral, serviceUUID: serviceUUID, operationUUID: characterUUID) { character, error in
            ...
            if (character?.descriptorArray.count ?? 0) > 0 {
    ...
    public func readDescriptorWithPeripheral(peripheral:TFYSwiftEasyPeripheral,serviceUUID:String,characterUUID:String,callback:blueToothOperationCallback?) {
        self.searchCharacteristicWithPeripheral(peripheral: peripheral, serviceUUID: serviceUUID, operationUUID: characterUUID) { character, error in
            if error != nil {
                callback?(nil,error)
                return
    ...
    /// 读取设备的rssi
    public func readRSSIWithPeripheral(peripheral:TFYSwiftEasyPeripheral,callback:TFYSwiftEasyPeripheral.blueToothReadRSSICallback?) {
    ...
    /// 主动断开已经连接成功的设备操作
    public func disconnectWithPeripheral(peripheral:TFYSwiftEasyPeripheral) {
        peripheral.disconnectDevice()
    }
    public func disconnectWithIdentifier(identifier:UUID) {
        if let tempPeripheral = self.centerManager.connectedDeviceDict[identifier.uuidString] as? TFYSwiftEasyPeripheral {
            tempPeripheral.disconnectDevice()
        }
    }
    public func disconnectAllPeripheral() {
    ...
    public func connectDeviceWithName(name:String,serviceUUID:String,notifyUUID:String,writeUUID:String,data:Data?,callback:blueToothOperationCallback?) {
    ...
    public func scanDeviceWithCondition(condition:Any?,callback:blueToothScanCallback?) {
        if condition == nil {
        ...
        self.centerManager.scanDeviceWithTimeInterval(timeInterval: self.managerOptions?.scanTimeOut, service: self.managerOptions?.scanServiceArray, options: self.managerOptions?.scanOptions) { [weak self] peripheral, searchType in
            ...
            if condition is String {
                ...
                if (peripheral.name?.contains(name) ?? false) && searchType == .searchFlagTypeAdded {
                    self?.centerManager.stopScanDevice()
    ...
    public func scanAllDeviceWithCondition(condition:Any?,callback:blueToothScanAllCallback?) {
        if self.managerOptions?.scanTimeOut == NSIntegerMax {
            self.managerOptions?.scanTimeOut = 20
        }
        ...
        self.centerManager.scanDeviceWithTimeInterval(timeInterval: self.managerOptions?.scanTimeOut, service: self.managerOptions?.scanServiceArray, options: self.managerOptions?.scanOptions) { [weak self] peripheral, searchType in
            ...
            if condition == nil {
                let isEixt:Bool = self?.isExitObject(peripheral: peripheral, tempArray: tempArray) ?? false
                if !isEixt {
                    tempArray.append(peripheral)
                }
            } else if condition is String {
                let name:String = condition as! String
                if (peripheral.name?.contains(name) ?? false) {
                    let isEixt:Bool = self?.isExitObject(peripheral: peripheral, tempArray: tempArray) ?? false
                    if !isEixt {
                        tempArray.append(peripheral)
                    }
    ...
    public func isExitObject(peripheral:TFYSwiftEasyPeripheral,tempArray:[Any]) -> Bool {
        var isExited:Bool = false
        for (_,obj) in tempArray.enumerated() {
            if let tempP = obj as? TFYSwiftEasyPeripheral,
               tempP.identifierString == peripheral.identifierString {
                isExited = true
                break
            }
        }
    ...
    public func dealScanedAllDeviceWithArray(deviceArray:[TFYSwiftEasyPeripheral],error:Error?,callback:blueToothScanAllCallback?) {
    ...
    public func searchCharacteristicWithPeripheral(peripheral:TFYSwiftEasyPeripheral?,serviceUUID:String,operationUUID:String,callback: blueToothFindCharacteristic?)  {
            ...
            if exitedService != nil {
                ...
                exitedService?.discoverCharacteristicWithCharacteristicUUIDs(uuidArray: [operationuuid], callback: { characteristics, error in
                    ...
                    characteristics.forEach { tempCharacter in
                        if tempCharacter.UUID?.isEqual(operationuuid) == true {

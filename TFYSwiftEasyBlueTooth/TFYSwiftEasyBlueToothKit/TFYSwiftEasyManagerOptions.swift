...
public class TFYSwiftEasyManagerOptions: NSObject {
    ...
    public var managerQueue:DispatchQueue?
    ...
    public var managerDictionary:[String:Any]? = [CBCentralManagerOptionShowPowerAlertKey:true]
    ...
    public var scanOptions:[String:Any]? = [CBCentralManagerScanOptionAllowDuplicatesKey:true]
    
    /// 连接设备所需的服务.
    public var scanServiceArray:[CBUUID]?
    
    /// 连接设备时所带的条件
    public var connectOptions:[String:Any]? = [CBConnectPeripheralOptionNotifyOnConnectionKey:true,
                                     CBConnectPeripheralOptionNotifyOnDisconnectionKey:true,
                                      CBConnectPeripheralOptionNotifyOnNotificationKey:true]
    
    /// 扫描所需时间。默认为永久
    public var scanTimeOut:Int? = NSIntegerMax
    
    /// 连接设备最大时长 默认为5秒
    public var connectTimeOut:Int? = 5
    
    /// 断开连接后重新连接
    public var autoConnectAfterDisconnect:Bool? = true
    ...
    /// 验证配置是否有效
    public func validateOptions() -> Bool {
        // 检查必要的配置项
